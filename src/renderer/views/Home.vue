...
import { reactive, computed, watch, onMounted, inject } from "vue"; // Added inject
...
// Inject i18n from App.vue if available, or fetch it again if needed.
// However, the original App.vue logic fetched it.
// To keep it simple and consistent with the original logic which was self-contained in App.vue,
// I will keep the fetching logic here as well for now, or rely on App.vue to pass it?
// The original App.vue handled everything. navigating away and back might loose state if not careful.
// For Home.vue, let's keep the logic self-contained as it was in App.vue for now.
...
const state = reactive({
  ...
  current: 0,
  showSetting: false,
  i18n: i18nFromApp?.value || null, // Initialize with injected value
  showUrlDlg: false,
...
// Watch for changes in injected i18n (e.g. initial load)
watch(
  () => i18nFromApp?.value,
  (newVal) => {
    if (newVal) {
...
const getI18nData = async () => {
  ...
  if (data) {
    state.i18n = data;
    // setTitle() // Title setting can remain in App.vue or be here
  }
};
