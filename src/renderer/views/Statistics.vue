                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    >
                      <div
                        ...
                        :class="getBarColor(record.pity)"
                        :style="{
                          width:
                            getPityPercentage(
                              record.pity,
                              activeTypeConfig.maxPity,
                            ) + '%',
                        }"
                      ></div>
...
import mockDataMapRaw from "../mock/gachaData.js"; // Map<poolType, records[]>
...
// Active Tab: 11 = Limited (特许), 1 = Standard (常驻)
const activeType = ref("11");
...
const toggleMock = () => {
  if (useMock.value) {
    rawDataMap.value = mockDataMapRaw;
  } else {
...
const bannerList = computed(() => {
  ...
  relevantPools.forEach((pool) => {
    ...
    const matchingRecords = activeRecords.value.filter((r) => {
    ...
    matchingRecords.forEach((r) => {
      if (r.rank_type === "6") {
        ...
        let tag = "";
        if (pity <= 10) tag = "超欧";
        else if (pity <= 30) tag = "欧";
        else if (pity >= 70) tag = "非";
    ...
    banners.push({
      ...
      totalPulls: matchingRecords.length,
...
const getPityPercentage = (val, max) => Math.min((val / max) * 100, 100);

const getBarColor = (pity) => {
  if (pity <= 20)
    return "bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.4)]";
  if (pity <= 50) return "bg-blue-400";
  if (pity <= 65) return "bg-amber-400";
  return "bg-red-500";
};

const getPityColorText = (pity) => {
  if (pity <= 20) return "text-emerald-600";
  if (pity <= 50) return "text-blue-600";
  if (pity <= 65) return "text-amber-600";
  return "text-red-600";
};

const getTagColor = (tag) => {
  if (tag === "超欧") return "bg-emerald-500 text-white shadow-emerald-500/20";
  if (tag === "欧") return "bg-blue-500 text-white shadow-blue-500/20";
  if (tag === "非") return "bg-red-500 text-white shadow-red-500/20";
...
onMounted(() => {
  useMock.value = true;
  toggleMock();
  // If no limited data, maybe switch to standard?
  // leave default
});
</script>
